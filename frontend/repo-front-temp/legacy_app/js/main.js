...
const expiryBoard = document.getElementById("expiry-board");
const expiryCollapseBtn = document.getElementById("expiry-collapse-btn");
...
const planBadges = document.querySelectorAll("[data-current-plan-badge]");
const planCta = document.getElementById("plan-cta");
const planLimitBanner = document.getElementById("plan-limit-banner");
const upgradeCtas = document.querySelectorAll("[data-upgrade-cta]");
const ENV_KEY = "gp_env";
const PLAN_KEY = "plan_type";
const PLAN_FREE = "free";
const PLAN_BUSINESS = "business";
const LOGIN_ENDPOINTS = ["https://myn8n.seommerce.shop/webhook/login"];
const ACCOUNT_BUSINESS_KEY = "gp_account_business";
...
function getPlanType() {
  const accountBusiness = localStorage.getItem(ACCOUNT_BUSINESS_KEY);
  if (accountBusiness !== null) {
    const isBusiness =
      accountBusiness === "1" ||
      accountBusiness === "true" ||
      accountBusiness === PLAN_BUSINESS;
    localStorage.setItem(PLAN_KEY, isBusiness ? PLAN_BUSINESS : PLAN_FREE);
  }
...
function updatePlanBadge() {
  if (!planBadges.length && !upgradeCtas.length) return;
  ...
  planBadges.forEach((badge) => {
    badge.textContent = business ? "Plano Business" : "Plano Free";
    badge.classList.toggle("plan-free", !business);
    badge.classList.toggle("plan-business", business);
    badge.setAttribute(
      "data-tooltip",
  ...
  upgradeCtas.forEach((cta) => {
    cta.style.display = business ? "none" : "inline-flex";
  });
}
...
const EXPIRY_COLLAPSE_KEY = "gp_expiry_board_collapsed";
let selectedCategory =
  localStorage.getItem(CATEGORY_FILTER_KEY) || CATEGORY_ALL_VALUE;

let expiryExpanded = false;
let expiryBoardCollapsed = localStorage.getItem(EXPIRY_COLLAPSE_KEY) === "1";
let lastExpiryItems = [];
...
let currentDetailExportItems = [];
let currentDetailExportTitle = "";
...
function openManagedModal(modalEl) {
  if (!modalEl) return;
  const timeoutId = modalCloseTimers.get(modalEl);
  if (timeoutId) {
...
function closeManagedModal(modalEl, { immediate = false } = {}) {
  if (!modalEl) return;
  const timeoutId = modalCloseTimers.get(modalEl);
  if (timeoutId) {
    clearTimeout(timeoutId);
  ...
  modalCloseTimers.set(modalEl, nextId);
}
...
function handleLogout() {
  ...
  localStorage.removeItem(ACCOUNT_BUSINESS_KEY);
  localStorage.removeItem("gp_account_id");
  localStorage.removeItem("gp_account_expira_em");
  localStorage.removeItem("gp_account_criado_em");
  localStorage.removeItem("gp_account_whatsapp");
  localStorage.removeItem("gp_account_loja");
  localStorage.removeItem(AUTH_KEY);
...
function getUserPayload() {
  return getUserPayloadForEmail(currentUserEmail);
}

function getUserPayloadForEmail(email) {
  const resolvedEmail = String(email || "").trim();
  if (!resolvedEmail) return {};
  return { user: resolvedEmail, email: resolvedEmail };
}
...
function extractLoginAccount(data) {
  if (!data) return null;
  if (Array.isArray(data)) {
    for (const item of data) {
      const account = extractLoginAccount(item);
      if (account) return account;
    }
    return null;
  }
  if (typeof data !== "object") return null;
  if (data.error || data.detail || data.status === "error") return null;
  const hasIdentity =
    Object.prototype.hasOwnProperty.call(data, "business") ||
  ...
  return extractLoginAccount(data.data || data.result || data.account);
}

async function verifyDeleteCredentials(email, password) {
  if (!email || !password) {
    return { ok: false, message: "Preencha email e senha." };
  }
  try {
    const res = await postJsonWithFallback(
      LOGIN_ENDPOINTS,
    ...
    if (isJson) {
      try {
        ...
        console.warn("Falha ao ler resposta de login:", err);
      }
    }
    const account = extractLoginAccount(data);
    if (!account) {
      return { ok: false, message: "Credenciais inválidas ou usuário não encontrado." };
    }
    return { ok: true, account };
  } catch (err) {
    return { ok: false, message: `Erro ao validar login: ${err.message || err}` };
  }
}
...
if (detailExportBtn) {
  detailExportBtn.addEventListener("click", exportDetailSelectionToXlsx);
}
...
function updateExpiryCollapseUI() {
  if (!expiryBoard || !expiryCollapseBtn) return;
  expiryBoard.classList.toggle("is-collapsed", expiryBoardCollapsed);
  expiryCollapseBtn.textContent = expiryBoardCollapsed ? "Expandir" : "Recolher";
  expiryCollapseBtn.setAttribute("aria-expanded", String(!expiryBoardCollapsed));
}

if (expiryCollapseBtn) {
  updateExpiryCollapseUI();
  expiryCollapseBtn.addEventListener("click", () => {
    expiryBoardCollapsed = !expiryBoardCollapsed;
    localStorage.setItem(EXPIRY_COLLAPSE_KEY, expiryBoardCollapsed ? "1" : "0");
    updateExpiryCollapseUI();
  });
}
...
function closeDetailModal(options = {}) {
  closeManagedModal(detailModal, options);
}
...
function renderDetailList(items) {
  ...
  items.forEach((product) => {
    ...
    if (!isArchived) {
      ...
      const makeBtn = (label, className, handler, { closeAfter = true } = {}) => {
        ...
        btn.addEventListener("click", async (event) => {
          event.stopPropagation();
          if (closeAfter) closeDetailModal();
          try {
            await handler();
          } catch (err) {
            console.error(err);
            if (statusEl) {
              statusEl.textContent = `Erro na ação: ${err?.message || err}`;
            }
          }
        });
        return btn;
      ...
      actions.appendChild(
        makeBtn(
          ...
          "btn-secondary",
          () => {
            closeDetailModal({ immediate: true });
...
function updateDetailExportButtonState() {
  if (!detailExportBtn) return;
  const count = currentDetailExportItems.length;
  detailExportBtn.disabled = count === 0;
  detailExportBtn.textContent = count
    ? `Exportar seleção (${count})`
    : "Exportar seleção";
}

function buildRowsFromDetailItems(items = []) {
  const headers = [
    "ID",
    "EAN",
    "NOME",
  ...
  const rows = items.map((product) => [
    ...
    product?.quantidade_text ?? product?.quantidade ?? "",
    product?.validade ?? "",
    booleanLabel(product?.troca, false),
    isProductSold(product) ? "SIM" : "NÃO",
    isProductRemoved(product) ? "SIM" : "NÃO",
    toBoolean(product?.lancado, false) ? "SIM" : "NÃO",
    booleanLabel(product?.rotatividade_alta, false),
    product?.data_lancado ?? "",
  ]);
  return [headers, ...rows];
}

function exportDetailSelectionToXlsx() {
  if (!isBusinessPlan()) {
    showPlanCta("Exporte seus dados em XLSX, CSV ou PDF com o plano Business");
    return;
  }
  if (!currentDetailExportItems.length) {
    statusEl.textContent = "Nenhum item no popup para exportar.";
    return;
  }
  const rows = buildRowsFromDetailItems(currentDetailExportItems);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, buildSheetFromRows(rows), "Detalhes");
  const current = getCurrentSheet();
  const baseName = slugifyFilename(current?.name || "planilha", "planilha");
  const contextName = slugifyFilename(
    currentDetailExportTitle || activeReportContext?.title || "detalhes",
    "detalhes"
  );
  const filename = `dados-${baseName}-${contextName}.xlsx`;
  XLSX.writeFile(wb, filename);
  statusEl.textContent = `Exportação concluída: ${filename}`;
}

function openDetailModal({ title, subtitle = "", items = [], reportHint = false }) {
  if (!detailModal) return;
  if (detailTitle) detailTitle.textContent = title || "Detalhes";
  if (detailSubtitle) {
    const reportHintText = reportHint
      ? " Use o botão Exportar seleção neste popup para baixar este recorte."
      : "";
    detailSubtitle.textContent = `${subtitle || ""}${reportHintText}`.trim();
  }
  const sorted = sortProductsForDetail(items);
  currentDetailExportItems = sorted.slice();
  currentDetailExportTitle = title || "";
...
function setActiveReportContext(key, title = "") {
  ...
  activeReportContext = {
    key: normalizedKey,
    title: String(title || "").trim() || normalizedKey,
  };
}

function slugifyFilename(value, fallback = "filtro") {
...
function getRowsForReportKey(key = "total") {
  const current = getCurrentSheet();
  if (!current) return [];
  const rows = XLSX.utils.sheet_to_json(current.sheet, {
    header: 1,
  ...
  if (!rows.length) return [];
  const normalizedKey = normalizeReportKey(key);
  if (normalizedKey === "total") return rows;
  ...
  dataRows.forEach((row) => {
    ...
    const payload = buildPayloadFromRow(headerRow, row);
    if (!payload) return;
    mapped.push({ row, payload });
  });

  const filteredPayloads = filterProductsByKey(
    mapped.map((item) => item.payload),
    normalizedKey
  );
  const keepSet = new Set(filteredPayloads);
  const filteredRows = mapped
    .filter((item) => keepSet.has(item.payload))
    .map((item) => item.row);
...
function getExportContext() {
  ...
  const reportKey = activeReportContext?.key || "total";
  const reportTitle = activeReportContext?.title || "";
  const rows = getRowsForReportKey(reportKey);
  return {
    current,
    rows,
    reportKey,
    ...
    filenameSuffix:
      reportKey === "total"
        ? ""
        : `-${slugifyFilename(reportTitle, reportKey)}`,
  };
}
...
function filterProductsByKey(products, key) {
  ...
  switch (normalizedKey) {
    ...
    case "sold":
      return products.filter((p) => isProductSold(p));
    case "removed":
      return products.filter((p) => isProductRemoved(p));
    case "history":
      return products.filter((p) => isProductSold(p) || isProductRemoved(p));
    case "launched":
...
function showDetailsForKey(key, title, subtitle) {
  ...
  setActiveReportContext(key, title);
  if (statusEl) {
    const normalized = normalizeReportKey(key);
    if (normalized === "total") {
      statusEl.textContent = "Filtro de relatório limpo. Exportando planilha completa.";
    } else {
      statusEl.textContent = `Filtro ativo: ${title} (${filtered.length} item(ns)).`;
    }
  }
  openDetailModal({ title, subtitle, items: filtered, reportHint: true });
}
...
function resolveExportContext() {
  ...
  if (context.rows.length < 2) {
    statusEl.textContent =
      context.reportKey === "total"
        ? "Nenhum item disponível para exportar."
        : `Nenhum item para o filtro "${context.reportTitle}".`;
    return null;
  }
  return context;
}
...
function buildExportFilename(context, extension) {
  const baseName = slugifyFilename(context.current?.name || "planilha", "planilha");
  return `dados-${baseName}${context.filenameSuffix}.${extension}`;
}

function exportCurrentSheetToPdf(context) {
  const exportContext = context || resolveExportContext();
  if (!exportContext) return;
  const { current, rows, reportTitle } = exportContext;
  ...
  win.document.write(`
    <html>
      <head><title>${escapeHtml(current.name)}${reportTitle ? ` - ${escapeHtml(reportTitle)}` : ""} - PDF</title>${styles}</head>
      <body>
        <h3>
          Planilha: ${escapeHtml(current.name)}
          ${reportTitle ? ` | Filtro: ${escapeHtml(reportTitle)}` : ""}
        </h3>
...
exportXlsxBtn.addEventListener("click", () => {
  ...
  XLSX.utils.book_append_sheet(
    wb,
    buildSheetFromRows(context.rows),
    context.current.name || "Dados"
  );
  XLSX.writeFile(wb, buildExportFilename(context, "xlsx"));
});
...
exportCsvBtn.addEventListener("click", () => {
  ...
  const context = resolveExportContext();
  if (!context) return;
  const csv = XLSX.utils.sheet_to_csv(buildSheetFromRows(context.rows));
  downloadBlob(
    csv,
    "text/csv;charset=utf-8;",
    buildExportFilename(context, "csv")
  );
});
...
exportJsonBtn.addEventListener("click", () => {
  ...
  const context = resolveExportContext();
  if (!context) return;
  const json = XLSX.utils.sheet_to_json(buildSheetFromRows(context.rows), {
    defval: "",
  });
  ...
  downloadBlob(
    ...
    buildExportFilename(context, "json")
  );
});
...
if (exportPdfBtn) {
  exportPdfBtn.addEventListener("click", () => {
    ...
    const context = resolveExportContext();
    if (!context) return;
    exportCurrentSheetToPdf(context);
  });
}

function requestDeleteCredentials() {
  const defaultEmail = currentUserEmail || localStorage.getItem("signup_email") || "";
  const emailInput = window.prompt(
    "Digite o email da conta para confirmar a exclusão:",
    defaultEmail
  );
  if (emailInput === null) return null;
  const email = (emailInput || "").trim();
  if (!email) {
    alert("Email obrigatório para eliminar produtos.");
    return null;
  }
  const passInput = window.prompt("Digite a senha de login para eliminar produtos:");
  if (passInput === null) return null;
...
async function deleteTargets(targets) {
  ...
  const credentials = requestDeleteCredentials();
  if (!credentials) {
    statusEl.textContent = "Eliminação cancelada.";
    return;
  }
  statusEl.textContent = "Validando login para eliminação...";
  const loginCheck = await verifyDeleteCredentials(credentials.email, credentials.password);
  if (!loginCheck.ok) {
    statusEl.textContent = loginCheck.message || "Credenciais inválidas.";
    return;
  }
  setBulkButtonsDisabled(true);
  ...
  try {
    ...
    for (const payload of list) {
      ...
      const res = await postJsonWithFallback(getEndpoints("actions"), {
        ...
        ...getUserPayloadForEmail(credentials.email),
      }, { retryOnHttpError: false });
      const result = await parseActionResponse(res);
      if (result.ok) {
        sent += 1;
...
async function enviarItem(item, index, statusEl) {
  ...
  const res = await postJsonWithFallback(getEndpoints("produto"), payload, { retryOnHttpError: false });
  const result = await parseActionResponse(res);
  if (!result.ok) {
...
const targets = currentEditTargets.length
  ...
  try {
    ...
    for (const target of targets) {
      ...
      const res = await postJsonWithFallback(getEndpoints("actions"), payload, { retryOnHttpError: false });
      const result = await parseActionResponse(res);
      if (result.ok) {
        sent += 1;
      } else {
...
function updateUpgradeToastVisibility() {
  const planType = localStorage.getItem(PLAN_KEY);
  const dismissed = localStorage.getItem("upgrade_toast_dismissed") === "1";
  const shouldHide = planType === PLAN_BUSINESS || dismissed;
  if (upgradeToast) {
    upgradeToast.classList.toggle("is-hidden", shouldHide);
  }
  document.body.classList.toggle("has-upgrade-toast", Boolean(upgradeToast) && !shouldHide);
}
